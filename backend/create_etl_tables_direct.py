"""Create ETL tables directly using SQL."""
...
CREATE TABLE IF NOT EXISTS etl_runs (
    ...
    triggered_by_user_id VARCHAR(36) REFERENCES users(id) ON DELETE SET NULL,
...
def create_tables():
    try:
        ...
        with engine.begin() as conn:
            # Execute SQL as single statement
            print("  Executing SQL...")
        ...
        if 'already exists' in error_str or 'duplicate' in error_str:
            print("✓ ETL tables already exist (or partially created)")
        else:
            print(f"✗ Error: {e}")
            import traceback
            traceback.print_exc()
            # Try to verify if tables exist despite error
            try:
                ...
                """))
                if result.scalar():
                    print("✓ But etl_pipelines table exists - you may be good to go!")
            except:
                pass
            sys.exit(1)
