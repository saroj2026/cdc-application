...
if ! command -v sshpass &> /dev/null; then
    ...
    if command -v brew &> /dev/null; then
        ...
        echo ""
        echo "Or run the manual steps provided."
        exit 1
    fi
fi
...
CONTAINER_JAR_COUNT=$(sshpass -p "$VPS_PASSWORD" ssh -o StrictHostKeyChecking=no "$VPS_USER@$VPS_HOST" "docker exec $CONTAINER_NAME find $CONTAINER_PLUGIN_PATH/$CONNECTOR_NAME -name '*.jar' 2>/dev/null | wc -l" | tr -d ' ')
...
sshpass -p "$VPS_PASSWORD" ssh -o StrictHostKeyChecking=no "$VPS_USER@$VPS_HOST" "docker exec $CONTAINER_NAME ls -la $CONTAINER_PLUGIN_PATH/$CONNECTOR_NAME | head -10"
echo ""
...
if [ -n "$PLUGIN_CHECK" ]; then
    ...
    echo "  sshpass -p '$VPS_PASSWORD' ssh $VPS_USER@$VPS_HOST 'docker logs $CONTAINER_NAME | tail -50 | grep -i error'"
fi
