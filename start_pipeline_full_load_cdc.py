"""Start pipeline and monitor full load -> CDC transition."""
...
def main():
    """Main function."""
    print("\n" + "="*60)
    print("START PIPELINE: FULL LOAD -> CDC".center(60))
    print("="*60 + "\n")
    ...
    print_status("\nStep 3: Checking initial status...", "INFO")
    ...
    print_status("\nStep 4: Starting pipeline (will trigger full load)...", "INFO")
    try:
    ...
    # Step 5: Monitor full load progress
    print_status("\nStep 5: Monitoring full load progress...", "INFO")
    ...
    for i in range(60):  # Monitor for up to 2 minutes
        ...
        # Check API status
        api_status = get_pipeline_status_api(pipeline_id)
        api_full_load = api_status.get('full_load_status')
        api_cdc = api_status.get('cdc_status')
        api_pipeline_status = api_status.get('status')
        
        # Check DB status
        db_status = get_pipeline_status_db(pipeline_id)
        db_full_load = db_status.get('full_load_status') if db_status else None
        db_cdc = db_status.get('cdc_status') if db_status else None
        db_pipeline_status = db_status.get('status') if db_status else None
        ...
        if db_full_load == "IN_PROGRESS" and not full_load_started:
            ...
            print_status(f"      DB Status: {db_pipeline_status}, Full Load: {db_full_load}, CDC: {db_cdc}", "INFO")
        ...
        if i % 5 == 0 and i > 0:
            print_status(
                f"[{i*2}s] Status: Pipeline={db_pipeline_status}, FullLoad={db_full_load}, CDC={db_cdc}",
                "INFO"
            )
    ...
    print_status("\nStep 6: Final status check...", "INFO")
    ...
    if final_db_status:
        if (final_api_status.get('full_load_status') == final_db_status.get('full_load_status') and
            ...
            print_status(
                f"\n⚠ Status mismatch between API and DB",
                "WARNING"
            )
            return 1
