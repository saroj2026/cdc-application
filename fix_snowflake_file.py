"""Fix the Snowflake connector file - download and install properly."""
import paramiko
import time
...
print("=" * 70)
print("Fixing Snowflake Connector File")
print("=" * 70)
print(f"Current file is only 554 bytes - need to download full file (~2-5 MB)")
print("=" * 70)
...
try:
    ...
    print(f"\n2. Downloading connector JAR (this will take 1-2 minutes)...")
    print(f"   URL: {CONNECTOR_URL}")
    
    # Use curl with better options
    stdin, stdout, stderr = ssh.exec_command(
        f"cd /tmp && rm -f {host_path} && curl -L --connect-timeout 30 --max-time 300 -o {host_path} '{CONNECTOR_URL}' 2>&1 && stat -c%s {host_path} || echo 'FAILED'"
    )
    ...
    for i in range(24):  # Wait up to 2 minutes
        ...
        if size.isdigit() and int(size) > 1000000:  # > 1MB
            size_mb = int(size) / 1024 / 1024
            print(f"   ✅ Download complete! Size: {size_mb:.2f} MB")
            break
        elif i % 6 == 0:  # Every 30 seconds
            print(f"   ... still downloading ({i*5} seconds)")
    
    # Verify download
    stdin, stdout, stderr = ssh.exec_command(f"test -f {host_path} && stat -c%s {host_path} || echo '0'")
    downloaded_size = stdout.read().decode('utf-8').strip()
    
    if not downloaded_size.isdigit() or int(downloaded_size) < 1000000:
        print(f"   ❌ Download failed or incomplete: {downloaded_size} bytes")
        print(f"   Trying alternative download method...")
        
        # Try wget
        stdin, stdout, stderr = ssh.exec_command(
            f"cd /tmp && wget --timeout=300 -O {host_path} '{CONNECTOR_URL}' 2>&1 | tail -1 && stat -c%s {host_path} || echo '0'"
        )
        time.sleep(60)
        stdin, stdout, stderr = ssh.exec_command(f"test -f {host_path} && stat -c%s {host_path} || echo '0'")
        downloaded_size = stdout.read().decode('utf-8').strip()
    ...
    if downloaded_size.isdigit() and int(downloaded_size) > 1000000:
        ...
        print(f"   ❌ Download failed: {downloaded_size} bytes")
