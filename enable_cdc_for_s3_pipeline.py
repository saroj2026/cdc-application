"""Enable CDC for S3 pipelines by changing mode from FULL_LOAD_ONLY to FULL_LOAD_AND_CDC."""
...
try:
    ...
    print("=" * 70)
    print("Enable CDC for S3 Pipelines")
    print("=" * 70)
    ...
    try:
        ...
        if not s3_pipelines:
            ...
            print("\n" + "-" * 70)
            response = input(f"\nDo you want to enable CDC for these {len(s3_pipelines)} pipeline(s)? (yes/no): ")
            ...
            if response.lower() in ['yes', 'y']:
                ...
                if updated_count > 0:
                    ...
                    print("\n⚠️  Note: You need to restart the pipeline for the changes to take effect.")
                    print("   The pipeline will now enable CDC after full load completes.")
                else:
